...
def _create_request(
    client, csrf, employee_id, leave_type_id, date_debut="2026-09-07", date_fin="2026-09-11"
):
    return client.post(
        "/api/leave-requests",
        json={
...
def test_dg_cannot_create_leave_request(client, dg_user, employee_a, leave_type):
    csrf = login(client, dg_user.email)
    resp = _create_request(client, csrf, employee_a.id, leave_type.id)
    assert resp.status_code == 403


def test_dg_cannot_approve_or_reject(client, hr_user, dg_user, employee_a, leave_type):
    hr_csrf = login(client, hr_user.email)
    request_id = _create_request(client, hr_csrf, employee_a.id, leave_type.id).json()["id"]
...
def test_dg_can_read_all_requests_company_wide(
    client, hr_user, dg_user, employee_a, employee_b, leave_type
):
    hr_csrf = login(client, hr_user.email)
    _create_request(client, hr_csrf, employee_a.id, leave_type.id, "2026-09-07", "2026-09-11")
    _create_request(client, hr_csrf, employee_b.id, leave_type.id, "2026-10-05", "2026-10-06")
...
def test_hr_can_create_on_anyones_behalf_and_approve(client, hr_user, employee_a, leave_type):
    hr_csrf = login(client, hr_user.email)
    resp = _create_request(client, hr_csrf, employee_a.id, leave_type.id)
...
def test_employee_list_endpoint_forces_own_employee_id(
    client, hr_user, employee_a, employee_b, employee_b_user, leave_type
):
    hr_csrf = login(client, hr_user.email)
    _create_request(client, hr_csrf, employee_a.id, leave_type.id)
    _create_request(client, hr_csrf, employee_b.id, leave_type.id)
...
def test_employee_without_responsable_can_self_submit(client, employee_a_user, leave_type):
    csrf = login(client, employee_a_user.email)
    resp = _create_request(client, csrf, employee_a_user.employee_id, leave_type.id)
    assert resp.status_code == 201


def test_employee_cannot_submit_for_a_colleague_without_responsable_capability(
    client, employee_a_user, employee_b, leave_type
):
    csrf = login(client, employee_a_user.email)
    resp = _create_request(client, csrf, employee_b.id, leave_type.id)
    assert resp.status_code == 403


def test_employee_in_department_with_responsable_cannot_self_submit(
    client, colleague_under_responsable_user, leave_type
):
    csrf = login(client, colleague_under_responsable_user.email)
    resp = _create_request(
        client, csrf, colleague_under_responsable_user.employee_id, leave_type.id
    )
    assert resp.status_code == 403


def test_leave_responsable_can_submit_for_own_department_colleague(
    client, responsable_user, colleague_under_responsable, leave_type
):
    csrf = login(client, responsable_user.email)
    resp = _create_request(client, csrf, colleague_under_responsable.id, leave_type.id)
    assert resp.status_code == 201


def test_leave_responsable_can_submit_for_self(client, responsable_user, leave_type):
    csrf = login(client, responsable_user.email)
    resp = _create_request(client, csrf, responsable_user.employee_id, leave_type.id)
    assert resp.status_code == 201


def test_leave_responsable_cannot_submit_for_other_department(
    client, responsable_user, employee_other_dept, leave_type
):
    csrf = login(client, responsable_user.email)
    resp = _create_request(client, csrf, employee_other_dept.id, leave_type.id)
    assert resp.status_code == 403


def test_leave_responsable_cannot_approve_or_reject(
    client, hr_user, responsable_user, colleague_under_responsable, leave_type
):
    resp_csrf = login(client, responsable_user.email)
    request_id = _create_request(
        client, resp_csrf, colleague_under_responsable.id, leave_type.id
    ).json()["id"]
...
def test_certificate_forbidden_before_approval_ok_after_for_owner(
    client, hr_user, employee_a_user, leave_type
):
    csrf = login(client, employee_a_user.email)
    request_id = _create_request(client, csrf, employee_a_user.employee_id, leave_type.id).json()[
        "id"
    ]
